...
fun main(args: Array<String>) {
    val options = PipelineOptionsFactory.fromArgs(*args).withValidation().`as`(
            MainOptions::class.java)
    val p = Pipeline.create(options)

    p
            .apply("read text files", TextIO.read().from(options.getInputFile()))
            .apply(
                    FlatMapElements.into(TypeDescriptors.strings()).via { line ->
                        line.split("[^\\p{L}]+")
                    }
            )
            .apply("filter by word length",
                   Filter.by(SerializableFunction<String, Boolean> { word -> !word.isEmpty() }))
            .apply("count words", Count.perElement<String>())
            .apply(
                    MapElements.into(
                            TypeDescriptors.strings()).via<KV<String, Long>> { wordCount -> wordCount.key + ": " + wordCount.value })
